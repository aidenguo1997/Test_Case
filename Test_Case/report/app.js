...
function cardDiv(feature, times){
    let div = "";
    let cardTemplate = "";
    let initialVer = feature.version.split('.');
    initialVer = initialVer[1] + initialVer[2]
    // console.log(initialVer)
    if (initialVer == "00"){
...
function createReport(data, testTimes){
    ...
    finalStatus = getStatus(failNum, "0")

    summary = 
    `
    <table class="table border border-dark text-center summary">
        <thead class="borders ${finalStatus.toLowerCase()}"">
            <tr>
...
let failed = "";
oXHR.onreadystatechange = createFinalReport;
